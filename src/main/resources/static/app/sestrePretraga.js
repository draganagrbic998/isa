...
<nav class="navbar navbar-icon-top navbar-expand-lg navbar-dark bg-dark">
  <a class="navbar-brand" href="#">PRETRAGA MEDICINSKOG OSOBLJA</a>
  ...
  <div class="collapse navbar-collapse" id="navbarSupportedContent">
	...
	<table class="table">
		...
		<tr v-for="s in sestre">
			<td>{{s.ime}}</td>
			<td>{{s.prezime}}</td>
			<td><button v-on:click="deleteS(s.id)" class="btn"><i class="fa fa-trash fa-2x"></i>Obrisi</button></td></tr>
	</table>	
		<h3>{{nemaRezultata}}</h3>
		</div>
	...
	methods: {
		...
		deleteS: function(id) {
			...
			.then(response => {
				alert("Sestra " + id + " uspesno obrisana!");
				this.$router.push("/adminHome");
			})
			.catch(error => {
				alert("Nije moguce brisanje!!");
			});
